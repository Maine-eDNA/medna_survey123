...
LibraryList<-c("data.table","dplyr","lubridate","stringr","ggplot2","scales", "zoo", "tidyr", "gridExtra", "sf")
...
survey_envmeas_join %>%
  filter(!if_any(c(envmeas_depth, site_id), is.na) & envmeas_depth > 50)
```

## Environmental measurements where System Type is Stream
```{r}
survey_envmeas_join %>%
  filter(!if_any(c(envmeas_depth, site_id), is.na) & system_type=="S")
```
...
```{r}
survey_envmeas_join %>%
  filter(!if_any(c(water_temp, site_id), is.na) & water_temp>50)
```
...
```{r}
ggplot(survey_envmeas_join %>%
         filter(!if_any(c(water_temp, site_id, envmeas_depth), is.na) & water_temp<50), 
...
#  Temperature, C
ggplot(survey_envmeas_join %>%
         filter(!if_any(c(water_temp, site_id), is.na) & water_temp<50), aes(x=site_id, y=as.numeric(water_temp), fill=site_id)) + 
  geom_boxplot() +
  ylab("Temperature (C)") +
  xlab("Site ID") +
  facet_grid(survey_month ~ .) +
...
#  Temperature, C
ggplot(survey_envmeas_join %>%
         filter(!if_any(c(water_temp, site_id), is.na) & water_temp<50), aes(x=system_type, y=as.numeric(water_temp), fill=system_type)) + 
  geom_boxplot() +
  ylab("Temperature (C)") +
  xlab("System") +
  ggtitle(plotTitle) +
...
# check distribution
p1<-ggplot(survey_envmeas_join %>%
         filter(!if_any(c(salinity, site_id), is.na)),aes(x=as.numeric(salinity))) + 
  ggtitle(plotTitle) +
  ylab("Density") +
...
# check distribution
p2<-ggplot(survey_envmeas_join %>%
         filter(!if_any(c(salinity, site_id), is.na) & salinity<50),aes(x=as.numeric(salinity))) + 
  ggtitle(plotTitle) +
  ylab("Density") +
  xlab("Practical Salinity Unit (PSU)") +
  geom_histogram(aes(y=..density..), colour="black", fill="white")+
...
```{r}
ggplot(survey_envmeas_join %>%
         filter(!if_any(c(salinity, site_id, envmeas_depth), is.na) & salinity<50), 
...
```{r}
survey_envmeas_join %>%
         filter(!if_any(c(turbidity, site_id), is.na) & turbidity>40)
```
...
# check distribution
p1<-ggplot(survey_envmeas_join %>%
         filter(!if_any(c(turbidity, site_id), is.na)),aes(x=as.numeric(turbidity))) + 
  ggtitle(plotTitle) +
  ylab("Density") +
...
# check distribution
p2<-ggplot(survey_envmeas_join %>%
         filter(!if_any(c(turbidity, site_id), is.na) & turbidity<40),aes(x=as.numeric(turbidity))) + 
  ggtitle(plotTitle) +
  ylab("Density") +
  xlab("Turbidity (Formazin Nephelometric Unit)") +
  geom_histogram(aes(y=..density..), colour="black", fill="white")+
...
```{r}
ggplot(survey_envmeas_join %>%
         filter(!if_any(c(turbidity, site_id, envmeas_depth), is.na) & turbidity<40), 
...
ggplot(survey_envmeas_join %>%
         filter(!if_any(c(turbidity, site_id), is.na)), aes(x=site_id, y=as.numeric(turbidity), fill=site_id)) + 
  geom_boxplot() +
...
#  turbidity, Formazin Nephelometric Unit
ggplot(survey_envmeas_join %>%
         filter(!if_any(c(turbidity, site_id), is.na)), aes(x=system_type, y=as.numeric(turbidity), fill=system_type)) + 
  geom_boxplot() +
  ylab("Turbidity (Formazin Nephelometric Unit)") +
  xlab("System") +
  ggtitle(plotTitle) +
...
```{r}
survey_envmeas_join %>%
  select(survey_date, site_id, general_location_name, conductivity, envmeas_depth, envmeas_instrument, system_type, ysi_notes, envmeas_notes, env_measurements) %>%
...
## Environmental measurements where par1 is greater than 40
```{r}
survey_envmeas_join %>%
         filter(!if_any(c(par1, site_id), is.na) & par1>1000)
```
