...
ggplot() + 
  ...
  geom_sf(data=site_ids_sfdf %>%
            dplyr::mutate(prj_sub = gsub(",", " ", projects)) %>%
...
png(outputPngFileName, height=25*nrow(rcdext_fmt_df), width=500*ncol(rcdext_fmt_df))
...
png(outputPngFileName,height=6,width=9, units='in', res=300)
create_map_sids(boundingbox_sids, countries_sfdf, WBD_sfdf, lakes_sfdf, rivers_sfdf, site_ids_sfdf, rcdext_df, title_var)
dev.off()
knitr::include_graphics(outputPngFileName)
...
```{r}
depth_var <- "depth_core_collected"
